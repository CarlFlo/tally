...
Schema 16 is the current database version. Migration tests must cover supported older schemas and backup restore/upgrade paths, including durable torrent automation runs, episode-to-infohash tracking, show policies, feedback, bad-infohash state, automation chain revisions, assignments, and replay snapshots. Chain tests must cover fixed-stage validation, administrator-only API access, show scope, assignment and deletion cascades, live-action and animated default selection/reset, scheduled query/filter application, dry-run side-effect suppression, bounded credential-free traces, and the vertical editor browser path.
...
Use deterministic local fixtures. Verify exact request semantics, authentication headers, redirect/error behavior, bounded responses, cancellation, and secret redaction. Feature toggles must be tested at both API and UI boundaries. Torrent automation has one scheduler-owned enabled state shared by Settings and System â†’ Jobs; it defaults off while experimental and enabling it from either surface requires the experimental-feature confirmation.
