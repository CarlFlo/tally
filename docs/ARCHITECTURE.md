...
Release-group identity is parsed from the release name. Uploader identity is used only when normalized provider metadata supplies it. Provider/indexer preference uses the normalized Jackett provider name and remains usable even when uploader metadata is absent. Release age is evaluated when Jackett supplies a publish timestamp. The configured release delay also acts as a minimum candidate upload age so newly uploaded releases can age while better candidates appear; missing or unparseable timestamps remain neutral, and the episode-air-time guard still prevents premature searches. Runtime-aware size profiling is a separate validity/ranking signal: Tally uses episode runtime when available and show runtime as fallback, divides candidate size by runtime, and evaluates MB/minute against separate live-action and animated ranges. Missing runtime or size is neutral. The active range may reject implausible outliers and its fit score may rank otherwise valid releases, but it never changes show/episode confidence. These rules are deployment-global, versioned settings and are included in each immutable Previous Runs settings snapshot.

Jackett may expose a magnet, a retrievable `.torrent`, or both. Tally retains both transport alternatives and prefers the inspectable `.torrent`. A magnet cannot expose its file list before metadata exchange, so automatic magnet fallback is metadata-only and remains `Unverified`; it is permitted only after High-confidence identity and all configured metadata-level filters pass. If a fetched `.torrent` reveals a meaningful payload or identity failure, that stronger evidence rejects the release and Tally does not bypass it by falling back to the magnet.
...
Deployment-global per-show download policy is `default`, `auto`, or `never`. A separate media-profile override is `auto`, `live`, or `animated`. `auto` classifies animation conservatively from persisted provider show type/genres, while an administrator can override the effective profile from the show page without rewriting provider metadata. The global search, download, and automation switches remain authoritative kill switches; per-show overrides cannot bypass them. Notification choices remain profile-owned and do not create additional download decisions.

Runs are serialized/deduplicated per episode with database constraints and eligibility checks. `no suitable result` is a normal terminal result and is retried with bounded backoff during the configured retry window rather than lowering acceptance standards. An episode already recorded as downloaded by automation is not submitted again.
...
Caches and environment-provided secrets are excluded. UI-managed credentials, automation settings, run state, show policy, per-show media-profile overrides, and bad-infohash feedback are durable application state and belong in protected backups.
