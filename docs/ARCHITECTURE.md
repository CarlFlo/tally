...
Release-group identity is parsed from the release name. Uploader identity is used only when normalized provider metadata supplies it. Provider/indexer preference uses the normalized Jackett provider name and remains usable even when uploader metadata is absent. These rules are deployment-global, versioned settings. Their exact values are included in each immutable Previous Runs settings snapshot so historical decisions remain explainable after configuration changes.
