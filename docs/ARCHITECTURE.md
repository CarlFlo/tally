...
Episode downloaded state is deployment-global because there is one shared media/download environment; watched state remains profile-owned. A manual downloaded toggle therefore changes the episode for every profile that follows the show, while watched/unwatched and watch-history clearing affect only the acting profile. Unreleased episodes cannot be marked watched or downloaded: individual writes reject setting either state to true before release, and bulk season/show updates always operate on the released subset only; clearing stale state remains allowed. Schema migration promotes an episode to globally downloaded when any legacy profile had previously marked it downloaded. The Downloads page still reflects client-reported torrent state, and merely submitting or automatically selecting a torrent does not by itself mark the episode downloaded; media-import/renaming lifecycle management remains out of scope.
