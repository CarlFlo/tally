...
| `internal/flows` | Experimental advanced flow registry, typed graph validation, current-definition persistence, dry-run execution, and bounded run snapshots |
...
Advanced Automation Flows are an administrator-only experimental editor and replay surface. They do not replace or intercept the scheduler-owned torrent automation pipeline. A saved flow contains a small typed acyclic graph, node configuration, and canvas positions. The backend registry validates node kinds, configuration, port compatibility, graph cycles, and bounded size before saving or replaying. Saving updates the flow's one current definition; its increasing revision is an optimistic concurrency token, not a user-visible version history. Each replay retains its own trigger event, definition snapshot, and sanitized node trace. An unsaved editor draft is recorded as revision 0 with its full snapshot. Deleting a flow also deletes its saved definition and recorded flow runs through database cascades and requires an explicit UI confirmation.

Flow ports carry Event, Text, Results, or Candidate values, with a distinct color for each type in the editor. Replace Text accepts exactly one Event, Text, or Candidate input and exposes the same type on its matching output. It can change the event show name, plain text, or a candidate name/provider in replay; it cannot change episode identity or the authoritative context used by the release evaluator. Log and Stop also accept exactly one input of any of the four types. Download accepts only a Candidate. The backend validates these alternatives and rejects a Replace Text output whose type differs from its connected input.

The flow inspector exposes the Replace Text target field for the connected input type; new connections set its default field explicitly, while older flows retain their implicit default. Replace Text trims leading and trailing whitespace by default, with a switch to turn trimming off. It previews deterministic event, query, and text transformations as the draft changes. A selected historical or custom event supplies the initial preview input; a previous replay can supply trace input for nodes after provider search. The inspector labels reused trace input, and nodes whose output depends on Jackett or the release evaluator require a new replay for authoritative output. Build Search Query accepts optional words before the show name and after the season and episode number. Has Results is deprecated in the node catalog because Jackett Search provides Results and No results paths and Best Torrent provides a No candidate path; saved flows using it remain editable and executable.

The editor places flow creation, selection, naming, event choice or creation, and Test in a left panel above the node palette. The canvas remains wide on desktop; when the viewport narrows, the Inspector moves below the canvas, and phone layouts stack the controls, canvas, and Inspector vertically.

Replay uses either a historical torrent automation run or a validated custom show-available event as the trigger fixture and performs fresh Jackett discovery through the existing provider path. A custom event supplies show name, season, and episode; the server assigns synthetic identifiers and trigger time. Both fixtures produce the same inspectable event snapshot and node trace, while custom runs have no source automation run. The filter node calls the shared release evaluator for episode identity and confidence. Search observations in flow history are limited to a bounded, credential-free candidate summary; provider URLs, magnets, and API keys are excluded. During a replay, Log Value writes its connected sanitized trace value to the activity log with a `flow_test_log` action and an explicit TEST marker. Download only describes the candidate; no replay submits a torrent, sends a notification, or changes episode state. The visual playback uses the recorded trace after the server run completes. This first slice does not select flows for live shows, execute scheduled flow actions, or capture live advanced-flow runs.
