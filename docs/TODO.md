...
- DONE: Library actions and administration milestone - show/card menus, watch-history clear, Shift-remove, coloured episode status and completion badges; searchable action logs; show names in jobs; Webhook/Discord services with templates, test delivery, master toggle, subscriptions, and persistent daily release delivery; live countdowns; role-based administration with transferable admin access; self-deletion Danger zone; server-persisted browser login appearance. All backend/race/vet, nine browser workflows, frontend/native/Docker builds, and local deployment checks pass. Schema 4 upgraded with a validated snapshot and retained both profiles and all three schedules. See `docs/VALIDATION.md`.
...
- DONE: Immutable non-reused profile IDs, backend-enforced administrator roles, preferences, re-encoded avatars, remembered/deleted-cookie handling.
