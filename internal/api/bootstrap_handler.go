...
func (s *Server) bootstrap(w http.ResponseWriter, r *http.Request, _ auth.Session) error {
	profiles, e := s.DB.Rows(r.Context(), "SELECT p.id,p.display_name,p.avatar,r.is_admin,EXISTS(SELECT 1 FROM local_credentials c WHERE c.profile_id=p.id) AS has_password FROM profiles p JOIN profile_roles r ON r.profile_id=p.id ORDER BY p.created_at,p.id")
	if e != nil {
		return e
	}
