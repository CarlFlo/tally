...
func TestClientManagementRequiresOperator(t *testing.T) {
	...
	if _, e := s.DB.Exec("INSERT INTO profiles VALUES('user1','Alex','mint',1)"); e != nil {
		t.Fatal(e)
	}
	if e := s.Auth.NewSession(context.Background(), recorder, httptest.NewRequest("GET", "/", nil), "user1", false); e != nil {
		t.Fatal(e)
	}
