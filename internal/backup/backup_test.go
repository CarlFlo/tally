...
func TestConsistentBackupRetentionAndRestore(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "avatars", "profile-saved-test.png"), []byte("avatar"), 0600)
	...
	if _, e = db.Exec(`INSERT INTO shows(id,name) VALUES('saved-show','Saved show');
		INSERT INTO profile_shows(profile_id,show_id,added_at,favorite) VALUES('profile-saved','saved-show',1,1);
		INSERT INTO application_settings(key,data) VALUES('search','{"providers":[{"id":"saved","api_key":"fixture-key"}]}');
		INSERT INTO show_actions(profile_id,external_id,name,desired,status,updated_at) VALUES('profile-saved',7,'Pending show',1,'queued',1);
		INSERT INTO jobs(key,type,schedule,enabled,paused,failures) VALUES('metadata','metadata','15 8 * * 1-5',0,1,3)`); e != nil {
		t.Fatal(e)
	}
	if _, e = db.Exec(`INSERT INTO activity_log(action,profile_id,message,created_at) VALUES('show_added','profile-saved','Added saved show',1);
 INSERT INTO inbox_state VALUES('profile-saved',1,0);
 INSERT INTO inbox_dismissals SELECT 'profile-saved',id FROM activity_log WHERE action='show_added';
 ...
 INSERT INTO notification_outbox(event_key,event,message,occurred_at,available_at) VALUES('release:fixture','episode_released','Release pending',1,2);`); e != nil {
	...
	var name string
	if e = restored.QueryRow("SELECT display_name FROM profiles WHERE id='profile-saved'").Scan(&name); e != nil || name != "Saved" {
		t.Fatal("profile not restored")
	}
	if _, e = os.Stat(filepath.Join(target, "avatars", "profile-saved-test.png")); e != nil {
	...
	for _, query := range []string{
		...
		"SELECT COUNT(*) FROM inbox_state WHERE profile_id='profile-saved' AND seen_id=1",
		"SELECT COUNT(*) FROM inbox_dismissals WHERE profile_id='profile-saved'",
		...
		"SELECT COUNT(*) FROM profile_shows WHERE profile_id='profile-saved' AND favorite=1",
		"SELECT COUNT(*) FROM application_settings WHERE json_extract(data,'$.providers[0].api_key')='fixture-key'",
		"SELECT COUNT(*) FROM show_actions WHERE profile_id='profile-saved' AND status='queued' AND desired=1",
