...
func TestVersionOneArchiveStillRestoresAndUpgrades(t *testing.T) {
	...
	defer db.Close()
	if _, e = db.Exec("DROP TABLE inbox_dismissals; DROP TABLE inbox_state; DROP INDEX episode_release_time; DROP TABLE activity_log; DROP TABLE browser_preferences; DROP TABLE notification_state; DROP TABLE notification_outbox; DROP TABLE application_settings; DROP TABLE show_actions; ALTER TABLE profile_shows DROP COLUMN favorite; ALTER TABLE jobs DROP COLUMN enabled; ALTER TABLE jobs DROP COLUMN paused; ALTER TABLE jobs DROP COLUMN failures; ALTER TABLE jobs DROP COLUMN revision; DROP TABLE download_client_settings; PRAGMA user_version=1; UPDATE profiles SET display_name='From version one' WHERE id='user0'"); e != nil {
		t.Fatal(e)
	}
	...
	_ = restored.QueryRow("SELECT display_name FROM profiles WHERE id='user0'").Scan(&profile)
