...
FROM alpine:3.24
RUN apk upgrade --no-cache && apk add --no-cache ca-certificates && addgroup -g 10001 tally && adduser -D -H -u 10001 -G tally tally && mkdir -p /config && chown tally:tally /config
